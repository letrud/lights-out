...
`intent/lights-out.intent.json` is the contract. Everything else in this repo exists to satisfy it.
...
| --- | --- | --- |
| `intent/lights-out.intent.json` | The contract: unit, dimensions, states, severity, standard, views | Humans, deliberately |
...
| `data/lights-out.data.json` | The instances | The collector, or a human for `manual` fields |
...
## How a component is wired, what the adapters do, what CI must exist

In `intent/` — `INTENT.md`, `SPEC.md`, `EVAL.md`. That is the specification; this file is only a pointer for someone working here interactively. A component is a file under `intent/components/`; `data/` and `site/` are generated by the automation on `main` and are never edited by hand or carried in a pull request.
...
3. Backfill `data/lights-out.data.json`. Unknown is `null`, or the intent's absent state (`idle`) — never an optimistic guess. A component that has never run must look like it has never run.
